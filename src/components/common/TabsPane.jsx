...
export default function AnimatedTabsSection({
  ...
  const variants = {
    ...
    center: {
      ...
      opacity: 1,
      transition: {
        x: { type: "tween", duration: 0.45, ease: "easeOut" },
  ...
  return (
    <div className={className}>
      <Tabs
        ...
        <TabsList className={`relative flex w-full gap-4 `}>
          {items.map((i) => (
            <TabsTrigger
              key={i.value}
              value={i.value}
            ...
            //   className={`rounded-lg  transition-all duration-300  ease-out py-2 ${active?"!text-primary":"!text-black"}`}
              className={cn(`rounded-lg  transition-all duration-300  ease-out py-2 ${activeItem?"!text-primary":"!text-black"}`)}
