...
export default function ModelTraining({ onCloseModal, useCaseName, modelName }) {
  ...
  useEffect(() => {
    if (progress >= 100 && useCaseName && modelName) {
      const timeout = setTimeout(() => {
        router.push(
          `/ai-studio/use-cases/${encodeURIComponent(
            useCaseName
          )}/${encodeURIComponent(modelName)}/version-01`
        );
      }, 500);

      return () => clearTimeout(timeout);
    }
  }, [progress, useCaseName, modelName, router]);
