...
<Link href={`/ai-studio/use-cases/${routeId}/${modelId}/${versionSlug}`}>
        ...
        >
          ...
          >
            ...
            <div
              className={cn(
                "flex items-center justify-between rounded-lg p-3 px-2 text-sm py-6  duration-500 bg-white dark:bg-background dark:group-hover:bg-background ease-out group-hover:bg-white border border-border-color-2"
              )}
            >
              <div className="flex items-center gap-6 ">
                <div className="w-5 h-5">
                  <LineGraph className="text-primary group-hover:text-primary transition-all duration-500 ease-out dark:text-foreground " />
                </div>
                <div className="flex flex-col gap-1">
                  <p className=" text-foreground text-xs">Accuracy:</p>
                  <p className="text-badge-green">{accuracy}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <div className="flex gap-2 items-end">
                  <p
                    className={`text-xs border px-3 flex gap-2 items-center py-2 rounded-full ${
                      status === "Deployed"
                        ? "border-primary"
                        : "border-primary"
                    }`}
                  >
                    <RocketIcon className="w-5 h-5 text-black" />
                    {status}
                  </p>
                </div>
              </div>
